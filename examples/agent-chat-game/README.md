...
This demo is **GIF 2** for QuorumRouter v0.1 public release.

It shows **Agent Chat mode** as an **explicit opt-in fixture**. A deterministic
transcript shows `Grok` and `GLM` alternating a few moves from a simple shogi
opening excerpt, then fading out before the full match.
...
- The status is `explicit opt-in fixture`.
...
- No live Agent Chat provider claim.
- No live Grok/GLM API call.
- No SafeLoop mutation or external execution.
...
`status: "explicit_opt_in_fixture"`, fixture agents, partial match turns,
fadeout note, no-external-call note, deterministic fixture note, and runtime
boundaries.

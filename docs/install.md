...
npm package: `create-quorum-router@0.1.12`; npm dist-tag: `latest -> 0.1.12`.
For a fixed package version:

```bash
npx --yes create-quorum-router@0.1.12 my-quorum-router-demo
cd my-quorum-router-demo
deno task smoke
```

The scaffold does not fetch remote code during creation and does not install
dependencies automatically. `0.1.12` includes deterministic advisory task
...
- Clone or checkout errors for `v0.1.12`: verify network access and the ref
  spelling.

...
npm package: `create-quorum-router@0.1.15`; npm dist-tag: `latest -> 0.1.15`.
For a fixed package version:

```bash
npx --yes create-quorum-router@0.1.15 my-quorum-router-demo
cd my-quorum-router-demo
deno task smoke
...
dependencies automatically. `0.1.15` adds a deterministic offline walkthrough of
...
- Clone or checkout errors for `v0.1.15`: verify network access and the ref
  spelling.
